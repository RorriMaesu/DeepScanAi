...
class ToolTip:
    ...
    def show_tooltip(self, event=None):
        ...
        self.widget.update_idletasks()   # Update widget to get correct height.
        label.update_idletasks()        # Update label to get correct height.
        x = self.widget.winfo_rootx() + 20
        # Position tooltip directly above the input field
        y = self.widget.winfo_rooty() - label.winfo_reqheight() - 5 # Adjust vertical offset as needed
...
class DiskAnalyzerGUI:
    ...
    def initialize_ai(self):
        if self.default_ai_provider == "Gemini":
            try:
                ...
                if not api_key:
                    # Show API key entry dialog if key not found
                    api_key = self.show_api_key_dialog()
                    if not api_key:  # User canceled
                ...
                self.model = None
                # Show API key dialog on failure
                if "API_KEY_INVALID" in str(e) or "API key not found" in str(e):
                    self.safe_after(0, self.show_api_key_dialog)
        else:
            self.ai_enabled = True

    def show_api_key_dialog(self):
        """Show a dialog for entering the Gemini API key and save it to .env file"""
        api_key_dialog = ctk.CTkToplevel(self.window)
        api_key_dialog.title("Gemini API Key")
        api_key_dialog.geometry("500x300")
        api_key_dialog.resizable(False, False)
        api_key_dialog.attributes("-topmost", True)
        ...
        result = [None]  # Use a list to store the result so it can be modified in nested functions
        ...
        save_checkbox = ctk.CTkCheckBox(
            api_key_dialog,
            text="Save API key for future use",
            variable=save_var,
            font=("Segoe UI", 12)
        )
        save_checkbox.pack(pady=(0, 20))

        button_frame = ctk.CTkFrame(api_key_dialog, fg_color="transparent")
        ...
        def on_submit():
            ...
            # Save to .env file if checkbox is checked
            if save_var.get():
                try:
        ...
        self.window.wait_window(api_key_dialog)
    ...
    def setup_gui(self):
        ...
        # Ensure we have enough columns for all buttons and they're properly weighted
        for col in range(16):  # Increased from 14 to 16 to ensure enough space
            if col == 0:  # Give more weight to the first column (status label)
                self.top_frame.columnconfigure(col, weight=1)
            else:
                self.top_frame.columnconfigure(col, weight=0)

        self.scan_btn = ctk.CTkButton(
            self.top_frame, text="Select Folder (Ctrl+O)", command=self.select_folder,
            width=140, fg_color="#1E90FF", hover_color="#1C90EE", text_color="#FFFFFF",
            font=("Segoe UI", 12)
        )
        ...
        # -------------------- RIGHT FRAME: AI ANALYSIS & CHATBOT --------------------
        self.right_frame = ctk.CTkFrame(self.middle_frame, fg_color="#2A2A2A", corner_radius=10)
        self.right_frame.grid(row=0, column=1, sticky="nsew")
        # Configure two rows: row 0 for the Analyze button (fixed height) and row 1 for the tabview (expandable)
        self.right_frame.rowconfigure(0, weight=0)
        self.right_frame.rowconfigure(1, weight=1)
        self.right_frame.columnconfigure(0, weight=1)

        # Place the Analyze with AI button at the top of the right frame
        self.analyze_btn = ctk.CTkButton(
            self.right_frame, text="Analyze with AI", command=self.trigger_ai_analysis, width=140,
            fg_color="#1E90FF", hover_color="#1C90EE", text_color="#FFFFFF",
            font=("Segoe UI", 12)
        )
        self.analyze_btn.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="ew")
        ToolTip(self.analyze_btn, "Run AI analysis on your scanned files to get insights and recommendations.", self)

        # Tabview for AI Analysis, History, and Chatbot placed below the Analyze button
        self.analysis_tabview = ctk.CTkTabview(self.right_frame, width=400)
        self.analysis_tabview.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
    ...
    def start_guided_tour(self):
        ...
        steps = [
            ...
            (self.analyze_btn, "Run AI analysis on your scanned files to get insights and recommendations.", "bottom"),
            (chatbot_btn, "Switch to the Chatbot tab to interact with the disk management assistant.", "bottom"),
            (self.chat_entry, "Enter your query here and press Enter or click 'Send' to interact with the assistant.", "top"),
        ...
        def calculate_popup_position(widget, popup_width, popup_height, preferred_pos):
            ...
            if preferred_pos == "bottom":
                ...
                arrow_text = "▲"  # Arrow pointing up
                arrow_y = 0
            elif preferred_pos == "top":
                x = widget_x + (widget_width - popup_width) // 2
                # For the chatbot input field, use a smaller gap so the popup appears just above
                if widget == self.chat_entry:
                    y = widget_y - popup_height - 2
                else:
        ...
        def show_step(index):
            ...
            # If the step is for the chatbot input, first select the Chatbot tab.
            if widget == self.chat_entry:
                self.analysis_tabview.set("Chatbot")
            ...
            # Adjust popup width based on the widget:
            if widget == self.chat_entry:
                popup_width = 600  # Wider popup for the chatbot input field
            elif widget in (self.min_size_entry, self.analyze_btn, self.help_btn):
                popup_width = 400
            else:
                popup_width = 350
            popup_height = 180
            ...
            next_btn = ctk.CTkButton(btn_frame, text="Next",
                                     command=lambda: [highlight_widget(widget, False),
                                                      tour_win.destroy(),
                                                      show_step(index+1)],
                                     fg_color="#1E90FF", hover_color="#1C90EE", text_color="#FFFFFF", font=("Segoe UI", 12))
    ...
    def delete_file(self, file_path: str):
        if messagebox.askyesno("Confirm Deletion", f"Delete file?\n{file_path}"):
            try:
                ...
                # Update data structures regardless of whether file exists
    ...
    def append_deepseek_response_analysis(self, message):
        ...
        if match:
            ...
            toggle_btn = ctk.CTkButton(toggle_frame, text="Show Thinking Process", command=toggle_details,
                                         font=("Segoe UI", 12), fg_color="#1E90FF", hover_color="#1C90EE", text_color="#FFFFFF")
                ...
                if "API_KEY_INVALID" in str(e) or "API key not found" in str(e):
                    ...
                    if api_key:
                        ...
                        try:
                            ...
                            self.logger.error(f"Gemini API retry failed: {e2}")
                            return f"Error: {e2}"
                    else:
                        return "API key not provided. Please try again."
                return f"Error: {e}"
